function [mod, el_cents, ep, en, crack_nd_i] = fn_add_crack_3d(mod, crack_vtcs, crack_fcs, varargin)
if isempty(varargin)
    cod = 0;
else
    cod = varargin{1}
...
[crack_fcs, eds, ed_fcs] = fn_consistent_facet_nodes(crack_fcs)
...
e0 = abs(d) < max_el_size * 2; %this is purely for computational efficiency to minimise number of elements considered
ep = (sign(d)) >= 0 & e0;
en = (sign(d)) < 0 & e0;

%WORKS TO HERE

ep_faces = fn_faces_from_els(mod.els(ep, :), el_typ_i(ep))
