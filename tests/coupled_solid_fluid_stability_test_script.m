...
paint_youngs_modulus = 3.1e9;
paint_poissons_ratio = 0.3;
paint_density = 1500;

solid_matl_i = 1;
%Material properties
matls(solid_matl_i).rho = paint_density; %Density
...
matls(solid_matl_i).D = fn_isotropic_stiffness_matrix(paint_youngs_modulus, paint_poissons_ratio); 
matls(solid_matl_i).col = hsv2rgb([2/3,0,0.80]); %Colour for display
matls(solid_matl_i).name = 'Solid';
matls(solid_matl_i).el_typ = 'CPE3'; %CPE3 must be the element type for a solid

fluid_matl_i = 2;
matls(fluid_matl_i).rho = 1000;
...
matls(fluid_matl_i).D = 1500 ^ 2 * 1000;
matls(fluid_matl_i).col = hsv2rgb([0.6,0.5,0.8]);
matls(fluid_matl_i).name = 'Fluid'; 
matls(fluid_matl_i).el_typ = 'AC2D3'; %AC2D3 must be the element type for a fluid
...
mod = fn_set_els_inside_bdry_to_mat(mod, water_bdry_pts, fluid_matl_i);
...
time_step = fn_get_suitable_time_step(matls, el_size) * 2.447; %for steel 2.446 is OK, 2.447 start of trouble!
time_step = fn_get_suitable_time_step(matls, el_size) * 1; %for paint!
...
figure(2);
semilogy(steps{1}.load.time, abs(sum(res{1}.dsps)));
hold on;
xlabel('Time (s)')

%Animate result
anim_options.norm_val = max(sum(abs(res{1}.fld(:,1:5))));
figure;
